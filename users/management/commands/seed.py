...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Admin user (enforce default credentials: admin / admin)
        admin_pw = 'admin'
        if not UserModel.objects.filter(username='admin').exists():
            UserModel.objects.create_superuser('admin', 'admin@example.com', admin_pw)
            self.stdout.write(self.style.SUCCESS('Created admin user admin/admin'))
        else:
            # Optionally reset password each seed to guarantee known creds in dev
            admin_user = UserModel.objects.get(username='admin')
            admin_user.set_password(admin_pw)
            admin_user.save(update_fields=['password'])
            self.stdout.write(self.style.WARNING('Reset admin password to default admin/admin'))
